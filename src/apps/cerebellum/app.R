...
# Colour the cells themselves, not the grid squares. The estimate is still the
# per-square mean - a square holds ~30 cells, which is what makes it stable - but
# painting it onto the real coordinates keeps the embedding at full resolution
# instead of quantising it into squares, at no cost in data. Cells where the gene
# was never detected stay grey, so "not detected" cannot be read as "low", and the
# ramp is square-root: most genes sit at zero nearly everywhere, and a linear ramp
# turns a marker confined to one population into a flat wash.
emap <- function(g, title = NULL) {
  v <- gvec(g); req(!is.null(v))
  m <- cells; m$val <- v[match(m$bin, bins$bin)]; m$val[is.na(m$val)] <- 0
  m <- m[order(m$val), ]                          # expressing cells drawn on top
  z <- m$val <= 0
  ggplot() +
    geom_point(data = m[z, ],  aes(D1, D2), colour = "#E3E7EA", size = .3, alpha = .55) +
    geom_point(data = m[!z, ], aes(D1, D2, colour = val), size = .3, alpha = .85) +
